...
(define (open-input-string string #!optional start end)
  (let* ((end
	  (if (default-object? end)
	      ...
	      (begin
		(guarantee index-fixnum? end 'open-input-string)
		(if (not (fix:<= end (ustring-length string)))
		    (error:bad-range-argument end 'open-input-string))
		end)))
	 (start
	  (if (default-object? start)
	      0
	      (begin
		(guarantee index-fixnum? start 'open-input-string)
		(if (not (fix:<= start end))
		    (error:bad-range-argument start 'open-input-string))
		start))))
...
(define (open-input-octets octets #!optional start end)
  (let* ((end
	  (if (default-object? end)
	      (ustring-length octets)
	      (begin
		(guarantee index-fixnum? end 'open-input-octets)
		(if (not (fix:<= end (ustring-length octets)))
		    (error:bad-range-argument end 'open-input-octets))
		end)))
	 (start
	  (if (default-object? start)
	      0
	      (begin
		(guarantee index-fixnum? start 'open-input-octets)
		(if (not (fix:<= start end))
		    (error:bad-range-argument start 'open-input-octets))
		start))))
    (let ((port
  ...
  (let ((index start))
    (make-non-channel-input-source
     (lambda ()
       (fix:< index end))
     (lambda (bv start* end*)
       (let ((n (min (fix:- end index) (fix:- end* start*))))
...
(define (string-out/extract-output! port)
  (let ((os (textual-port-state port)))
    (let ((output (ustring-copy (ostate-buffer os) 0 (ostate-index os))))
      (reset-buffer! os)
      output)))
  ...
  (let loop ((index end))
    (and (fix:> index start)
	 (let ((index (fix:- index 1)))
	   (if (char=? (ustring-ref string index) #\newline)
	       index
	       (loop index))))))

;;;; Output as octets
