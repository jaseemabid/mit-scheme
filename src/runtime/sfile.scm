  ...
  ((ucode-primitive file-access 2)
   (let ((pathname (merge-pathnames filename)))
     (let ((filename (string->utf8 (->namestring pathname))))
	   ...
	   filename
	   (string->utf8 (directory-namestring pathname)))))
   2))
...
(define (rename-file from to)
  ((ucode-primitive file-rename)
   (string->utf8 (->namestring (merge-pathnames from)))
   (string->utf8 (->namestring (merge-pathnames to)))))
...
(define (file-eq? x y)
  ((ucode-primitive file-eq?)
   (string->utf8 (->namestring (merge-pathnames x)))
   (string->utf8 (->namestring (merge-pathnames y)))))
...
(define (directory-file-names directory #!optional include-dots?)
  (let ((channel
	 (directory-channel-open
	  (string->utf8 (->namestring (pathname-as-directory directory)))))
	(include-dots?
...
(define (allocate-temporary-file pathname)
  (and (not (file-exists? pathname))
       (let ((updater (fixed-objects-updater 'files-to-delete))
	     (filename (string->utf8 (->namestring pathname))))
