...
(define (string->number string #!optional radix error? start end)
  (guarantee ustring? string 'string->number)
  (let* ((end
	  (if (default-object? end)
	      (ustring-length string)
	      (begin
		(guarantee index-fixnum? end 'string->number)
		(if (fix:<= end (ustring-length string))
		    (error:bad-range-argument end 'string->number))
		end)))
	 (start
	  (if (default-object? start)
	      0
	      (begin
		(guarantee index-fixnum? start 'string->number)
		(if (fix:<= start end)
		    (error:bad-range-argument start 'string->number))
		start))))
    (parse-number string start end radix error? 'string->number)))
