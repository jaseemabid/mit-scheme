...
(define (string->parser-buffer string #!optional start end)
  (let* ((end
	  (if (or (default-object? end) (not end))
	      ...
	      (begin
		(guarantee index-fixnum? end 'string->parser-buffer)
		(if (not (fix:<= end (ustring-length string)))
		    (error:bad-range-argument end 'string->parser-buffer))
		end)))
	 (start
	  (if (or (default-object? start) (not start))
	      0
	      (begin
		(guarantee index-fixnum? start 'string->parser-buffer)
		(if (not (fix:<= start end))
		    (error:bad-range-argument start 'string->parser-buffer))
		start))))
	...
	(if (fix:> index 0)
	    (let* ((end* (fix:- end index))
		 ...
		 (do ((i index (fix:+ i 1))
		      (j 0 (fix:+ j 1)))
		     ((not (fix:< i end)))
		   (ustring-set! string* j (ustring-ref string i)))
...
(define (%grow-buffer string end min-length)
  (let ((new-string
    ...
    (do ((i 0 (fix:+ i 1)))
	((not (fix:< i end)))
      (ustring-set! new-string i (ustring-ref string i)))
    new-string))