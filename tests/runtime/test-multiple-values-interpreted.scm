...
(define (interpreted-producer)
  (let ((a 1)
	(b 2)
	(c 3))
    (values a b c)))

(define (interpreted-producer-mismatch extra)
  (cons 'interpreted extra))

(define (interpreted-consumer a b c)
  (note "a: "a" b: "b" c: "c)
  'interpreted-done)

(define (interpreted-consumer-mismatch a b)
  (note "mismatch: a: "a" b: "b)
  (cons a b))

(define (interpreted-caller)

  (define (test-note producer consumer)
    (note "caller: interpreted, producer: "producer", consumer: "consumer))
...
(define (interpreted-arity-errors)
  (assert-error
